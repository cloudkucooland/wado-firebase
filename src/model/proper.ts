...
export default class proper {
	...
	private _getSeason(forday: Date): void {
		...
		if (t < f('epiphany')) {
			...
			const daysintoordtime = this.getDayOfYear(forday) - this._fdoy('epiphany') + 1;
			...
			this.season = 'lent';
			const daysintolent = this.getDayOfYear(forday) - this._fdoy('ashwednesday') - 1;
			...
			const daysaftereaster = this.getDayOfYear(forday) - (this._fdoy('easter') + 1);
			...
			this.season = 'trinity';
			this.proper = 0;
		} else if (t >= f('trinity') + nextday * 6 && t >= f('proper0') && t < f('sept4')) {
	...
	get propername(): string {
		...
		switch (this.season) {
			case 'advent':
				return this.cardToOrd(this.proper) + ' ' + this._weekdayDisplay() + ' of Advent';
			case 'christmaseve':
			...
			case 'afterepiphany':
				return 'Epiphany (ordinary) ' + this.proper + ', ' + this._weekdayDisplay();
			case 'mardigras':
				return 'Mardi Gras';
			case 'ashwednesday':
			...
			case 'lent':
				...
				return this.cardToOrd(this.proper) + ' ' + this._weekdayDisplay() + ' ' + prep + ' Lent';
			case 'palmsunday':
				return 'Palm Sunday';
			case 'holyweek':
			...
			case 'greatfifty':
				return this.cardToOrd(this.proper) + ' ' + this._weekdayDisplay() + ' of Easter';
			case 'ascensioneve':
			...
			case 'afterpentecost':
				if (this.weekday != 0)
					return 'Proper ' + this.proper + ' after Pentecost: ' + this._weekdayDisplay();
				{
			...
			case 'beforeadvent':
				if (this.weekday != 0)
					return 'Proper ' + this.proper + ' before Advent: ' + this._weekdayDisplay();
			...
			while (i <= v.maxProper) {
				...
				if (v.name == 'postascension') {
					...
					while (d < 7 + v.startWeekday) {
						...
						if (v.maxWeekday != 0 && i >= v.maxProper && d > v.maxWeekday) break;
						obj.weekday = d % 7;
						const p = new proper(obj);
						ll.set(p.propername, p);
						d = d + 1;
